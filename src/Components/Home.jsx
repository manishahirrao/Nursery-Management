    ...
    <>
      ...
      <div className="absolute inset-0 flex flex-col items-start justify-center pl-32 w-[800px]">
        ...
        <p className="text-white text-xl font-semibold bg-opacity-50">
          ...
          outdoor wonders, we have everything you need to create your own green
          paradise.
        </p>
      ...
      <div className="flex flex-col items-center my-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-6">
          {item.map((cat) => (
            <div
              key={cat.id}
              className="relative w-48 h-48 rounded-lg overflow-hidden shadow-md cursor-pointer hover:scale-105 transition-transform group"
              onClick={() =>
            ...
            >
              <img
                ...
                alt={cat.name}
              />
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded bg-black bg-opacity-0 group-hover:bg-opacity-60 text-white text-center transition-all duration-200 pointer-events-none">
                {cat.name}
              </div>
            </div>
          ))}
      ...
      <div className="flex flex-col items-center my-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-6">
          {featuredProducts.map((cat) => (
            <div
              key={cat.id}
              className="relative w-48 h-48 rounded-lg overflow-hidden shadow-md cursor-pointer hover:scale-105 transition-transform"
            ...
            >
              <img
                ...
                alt={cat.name}
              />
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded bg-black bg-opacity-60 text-white text-center pointer-events-none">
                {cat.name}
              </div>
            </div>
          ))}
      ...
      <div className="flex items-center justify-between px-40 my-20 gap-20">
        ...
        <div className="flex flex-col gap-4 items-center">
          ...
          <p className="text-center">Expert care guides for all your plants</p>
        </div>

        <div className="flex flex-col gap-4 items-center">
          <FaShieldAlt className="text-4xl text-green-600" />
          <p className="font-semibold text-2xl">Guarantee</p>
          <p className="text-center">30-day plant health guarantee</p>
        </div>
      </div>
    </>
  );
